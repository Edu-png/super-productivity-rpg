...
- `dispatchAction(action)` — Dispatch NgRx actions from the controlled allowlist.
  Focus plugins may dispatch `[FocusMode] Show Overlay`,
  `[FocusMode] Hide Overlay`, `[FocusMode] Set Mode`, and
  `[FocusMode] Start Session`; the start action accepts an optional `duration`
  in milliseconds.
