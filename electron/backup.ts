...
// Separate, small ring of RPG character bundle backups (profile + Academia
// Arcana + Arcane Library + Game Library + habits, same shape as the manual
// "Baixar backup deste personagem" download) - independent of the main
// AppDataComplete backup ring above, on its own daily-triggered cadence (see
// RpgCharacterBackupService), kept to a fixed small file count so it can
// never grow unbounded.
export const RPG_BACKUP_DIR = path.join(app.getPath('userData'), 'rpg-backups');
const RPG_BACKUP_MAX_FILES = 7;
...
export function initBackupAdapter(): void {
  ...
  // RPG_BACKUP
  ipcMain.handle(IPC.RPG_BACKUP, rpgBackupData);

  // RPG_BACKUP_PICK_FOLDER: lets the user point the RPG character backup at
  // an extra folder (e.g. a local Google Drive/OneDrive sync folder) so the
  // desktop sync client uploads it for them - no Google API integration,
  // just writing where that client already watches.
  ipcMain.handle(IPC.RPG_BACKUP_PICK_FOLDER, async (): Promise<string | undefined> => {
    try {
      const win = getWin();
      const { canceled, filePaths } = await dialog.showOpenDialog(win, {
        title: 'Escolher pasta para backup automático do personagem',
      ...
      // Never allow writing inside the app's own private data dir - same
      // rule the sync-folder picker enforces (file-path-guard.ts).
      assertPathOutside(app.getPath('userData'), canonical);
      return canonical;
    } catch (e) {
      log('RPG_BACKUP_PICK_FOLDER failed');
      error(e);
      return undefined;
    }
  });
...
interface RpgBackupArgs {
  fileName: string;
  data: unknown;
  // Optional second destination (e.g. a local Google Drive/OneDrive sync
  // folder the user picked via RPG_BACKUP_PICK_FOLDER) - written and rotated
  // independently of RPG_BACKUP_DIR, best-effort (a failure here never blocks
  // the always-on local write).
  extraDir?: string;
}

const isRpgBackupArgs = (arg: unknown): arg is RpgBackupArgs =>
  !!arg &&
  typeof arg === 'object' &&
  typeof (arg as { fileName?: unknown }).fileName === 'string';

// eslint-disable-next-line prefer-arrow/prefer-arrow-functions
function writeRpgBackupToDir(dir: string, fileName: string, data: unknown): void {
  if (!existsSync(dir)) {
    mkdirSync(dir, { recursive: true });
  }
  writeFileSync(path.join(dir, fileName), JSON.stringify(data));
  const files = readdirSync(dir).filter((f) => f.endsWith('.json'));
  const filesWithMtime = files.map((name) => {
    const fp = path.join(dir, name);
    return { fileName: name, filePath: fp, mtime: statSync(fp).mtime.getTime() };
  });
  for (const file of selectBackupFilesToDelete(filesWithMtime, RPG_BACKUP_MAX_FILES)) {
    try {
      unlinkSync(file.filePath);
    } catch (e) {
      log(`Error deleting RPG backup file ${file.fileName}`);
      error(e);
    }
  }
}
...
// eslint-disable-next-line prefer-arrow/prefer-arrow-functions
function rpgBackupData(ev: IpcMainInvokeEvent, args: unknown): void {
  if (!isRpgBackupArgs(args)) {
    throw new Error('RPG_BACKUP: invalid arguments');
  }
  // fileName is generated by RpgCharacterBackupService (a timestamp, never
  // user input), but constrain it anyway so a compromised renderer can't
  // write outside the target dir via a crafted "../" name.
  const fileName = path.basename(args.fileName);
  try {
    writeRpgBackupToDir(RPG_BACKUP_DIR, fileName, args.data);
  } catch (e) {
    log('Error while backing up RPG characters');
    error(e);
  }
  if (args.extraDir) {
    try {
      const canonical = realpathSync.native(args.extraDir);
      assertPathOutside(app.getPath('userData'), canonical);
      writeRpgBackupToDir(canonical, fileName, args.data);
    } catch (e) {
      log('Error while backing up RPG characters to extra folder');
      error(e);
    }
  }
}
